...
var CakeHR = function Constructor(settings) {
    this.settings = settings;
};

// Prameters are mapped 1-1 to the official API
// see here for more information: http://docs.cakehr.apiary.io/
CakeHR.prototype.getTimeOff = function(parameters) {
	var endpoint = '/calendar/?';

	return rp({
		uri: baseUrl + this.settings.token + endpoint,
		qs: parameters,
		json: true
	});
};

CakeHR.prototype.timeOffsToString = function(timeOffs, separator) {
	separator = separator || '\n';
	var text = [];
